...
const checks = [
 ...
 ['server rules use custom claim', rules.includes('request.auth.token.server == true') && !rules.includes("request.auth.token.email == 'system-server@restaurantos.app'")],
  ['server identity is Admin-issued', read('src/server/invitationAuth.ts').includes('adminAuth.setCustomUserClaims') && read('src/server/invitationAuth.ts').includes('adminAuth.createCustomToken') && read('src/server/invitationAuth.ts').includes('signInWithCustomToken') && read('src/server/firebaseAdmin.ts').includes("getAuth as getAdminAuth") && !read('src/server/invitationAuth.ts').includes('createUserWithEmailAndPassword') && !read('src/server/invitationAuth.ts').includes('signInWithEmailAndPassword')],
  ['subscription browser path cannot self-activate', /if \(typeof window !== 'undefined' && !isTest\)/.test(read('src/services/subscriptionService.ts')) && read('src/services/subscriptionService.ts').includes("Subscription verification service is unavailable. Payment was not activated.") && read('src/services/subscriptionService.ts').includes('await setDoc(subDocRef, updatedSubscription')],
 ...
 ['Pages API URL has a trusted HTTPS production fallback', read('.github/workflows/deploy.yml').includes('VITE_API_BASE_URL="${VITE_API_BASE_URL:-https://restaurantos-xqi52dpwga-el.a.run.app}"')],
...
checks.push(
  ...
  ['production startup does not block on trusted server auth provisioning', !serverSource.includes('Fatal: backend server identity could not be initialized') && /await ensureServerAuthenticated\(\)/.test(serverSource) && serverSource.includes('must never prevent')],
  ...
  ['Pages workflow resolves trusted public production endpoints', releaseWorkflow.includes('VITE_API_BASE_URL:-https://restaurantos-xqi52dpwga-el.a.run.app') && releaseWorkflow.includes('VITE_PUBLIC_APP_URL:-https://restaurantos01.ai.studio')],
