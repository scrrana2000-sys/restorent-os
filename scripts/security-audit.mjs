...
const checks = [
 ...
 ['server rules use custom claim', rules.includes('request.auth.token.server == true') && !rules.includes("request.auth.token.email == 'system-server@restaurantos.app'")],
  ...
  ['client subscription service has no Firestore mutation path', !/\b(setDoc|updateDoc|deleteDoc|addDoc)\s*\([^\n]*subscription/i.test(read('src/services/subscriptionService.ts')) && !read('src/services/subscriptionService.ts').includes("subscription', 'current')`);\\n  await setDoc")],
