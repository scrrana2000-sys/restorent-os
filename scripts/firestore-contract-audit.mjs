...
const checks = [
  ...
  ['API routing separates local, production Cloud Run, and static hosting correctly', apiConfig.includes('PRODUCTION_API_BASE_URL') && apiConfig.includes('PRODUCTION_API_HOST') && apiConfig.includes('isLocal') && apiConfig.includes('isProductionCloudRun') && apiConfig.includes('baseUrl = PRODUCTION_API_BASE_URL') && apiConfig.includes('window.location.origin')],
