...
export const PaymentDueCenterModal: React.FC<PaymentDueCenterModalProps> = ({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-150 my-6 flex flex-col max-h-[90vh]">
        {/* Header */}
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row gap-3 shrink-0">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs bg-white border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            />
            {searchQuery && (
              <button
                type="button"
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-3.5 flex-1 bg-slate-100/50">
          {/* Error State */}
          {error ? (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center space-y-3 my-4">
              ...
              {onRetry && (
                <button
                  type="button"
                  onClick={onRetry}
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition-colors shadow-xs"
                >
            ...
            filteredOrders.map((ord) => {
              ...
              return (
                <div
                  key={ord.id}
                  className={`bg-white rounded-2xl p-4 border transition-all shadow-xs hover:shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                    isServed || isReady
                ...
                >
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {/* Order Type Badge */}
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-bold uppercase tracking-wider ${
                      ...
                      >
                        {isServed
                          ...
                          : isReady
                          ? 'READY • PAYMENT DUE'
                          : `${ord.status.toUpperCase()} • PAYMENT DUE`}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                      {ord.customerSnapshot?.name && (
                        <span className="flex items-center gap-1 text-slate-700 font-medium">
                          ...
                          <span>{ord.customerSnapshot.name}</span>
                        </span>
                      )}
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 shrink-0">
                    <div className="text-left sm:text-right space-y-0.5">
                      <div className="flex items-center sm:justify-end gap-2 text-xs text-slate-500">
                        <span>Total: {formatMoney(ord.grandTotalMinor, symbol)}</span>
                        <span>•</span>
                        <span>Paid: {formatMoney(ord.paidAmountMinor || 0, symbol)}</span>
                      </div>
                      <div className="text-sm sm:text-base font-black text-amber-600 flex items-center sm:justify-end gap-1">
                        <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Due:</span>
                        <span>{formatMoney(dueMinor, symbol)}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {hasPermission(role, 'cancel_orders') && (
                        <button
                          ...
                          className="px-3 py-2 bg-rose-50 hover:bg-rose-100 active:bg-rose-200 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold transition-all flex items-center gap-1 shrink-0"
                          title="Cancel Order"
                        >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 active:bg-indigo-800 transition-all shadow-sm flex items-center gap-1.5 shrink-0"
                      >
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500 shrink-0">
          <span>Realtime Payment Due Collection Center</span>
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-slate-200 text-slate-800 font-bold rounded-xl hover:bg-slate-300 transition-colors"
          >
            Close
          </button>
        </div>
